             ...
             implements BSTInterface<T>
{
  ...
  // logical predecessor and the predecessor's node is removed.
  {
    ...
    if (tree.getLeft() == null)////////////////cuando tiene cero hijos o un hijo
      return tree.getRight();
    else if (tree.getRight() == null) 
      return tree.getLeft();
    else/////////////////cuando tiene dos hijos
    {
      data = getPredecessor(tree.getLeft());///obtener el mas grande de los chicos, la info
      tree.setInfo(data); ///////poner en elnodo a eliminar
      tree.setLeft(recRemove(data, tree.getLeft()));  ////////eliminar el hijo malvado :O
      return tree;
    }
  }
  ...
  {
    if (tree == null)/////////buscar al nodo
      found = false;
    else if (element.compareTo(tree.getInfo()) < 0)
      tree.setLeft(recRemove(element, tree.getLeft()));
    else if (element.compareTo(tree.getInfo()) > 0)
      tree.setRight(recRemove(element, tree.getRight()));
    else  ////////despuesde encontrar al nodo, lo remueve
    {
